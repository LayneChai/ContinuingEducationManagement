...
import { getAdminCertificatesApi, reviewAdminCertificateApi } from '../../../api/certificate'
...
async function loadApplications() {
  loading.value = true
  try {
    applications.value = await getAdminCertificatesApi(statusFilter.value !== undefined ? { status: statusFilter.value } : undefined)
  } finally {
    loading.value = false
  }
}
...
async function submitReview() {
  ...
  try {
    await reviewAdminCertificateApi(reviewDialog.applyId, reviewDialog.form)
...
<template>
  <div class="page-wrap admin-certificate-page">
    <div class="page-heading">
      <div>
        <h1>证书审核</h1>
        <p>查看学员证书申请记录，并完成通过、驳回与发证信息维护。</p>
      </div>
...
.admin-certificate-page { gap: 18px; }
